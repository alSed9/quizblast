...
function HostGamePage() {
  ...
  // Tout extraire avec des valeurs par défaut
  const players = game?.players || []
  const gamePhase = game?.gamePhase || 'playing'
  const allFinished = game?.allFinished || false

  console.log('HostGamePage render:', { gamePhase, playersCount: players.length, allFinished })

  useEffect(() => {
    console.log('HostGamePage useEffect:', { allFinished, gamePhase })
    if (allFinished || gamePhase === 'final') {
      console.log('Redirection vers final')
      navigate(`/host/${roomCode}/final`)
    }
  }, [allFinished, gamePhase, navigate, roomCode])

  const finishedCount = players.filter(p => p && p.finished).length
  ...
  return (
    <div className="bg-background text-on-background min-h-screen font-body-lg flex flex-col">
      ...
      <header className="w-full h-16 bg-surface shadow-sm border-b border-outline-variant flex items-center justify-between px-4 shrink-0">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-4 max-w-3xl mx-auto w-full">
        ...
        <div className="w-full bg-surface border border-outline-variant rounded-2xl p-6 shadow-sm">
          ...
          {sortedPlayers.length > 0 ? (
            <div className="space-y-3">
              {sortedPlayers.map((player, index) => (
                <div 
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl ${player?.finished ? 'bg-secondary-container/20' : 'bg-surface-container-low'}`}
                >
            ...
            <div className="text-center py-8">
              <p className="font-body-lg text-on-surface-variant">En attente des joueurs...</p>
            </div>
          )}
          ...
          {allFinished && (
            <div className="text-center mt-8">
              ...
              <button
                ...
                className="bg-primary text-on-primary px-8 py-4 rounded-xl font-headline-md shadow-sm hover:bg-primary-fixed-variant transition-all active:scale-95"
              >
