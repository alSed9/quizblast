...
function PlayerWaitingNextPage() {
  ...
  const { currentPlayer, getRanking, gamePhase, currentQuestionIndex } = useGame()
  ...
  // Points pour atteindre le podium
  const pointsToPodium = playerRank > 3 && ranking[2] 
    ? ranking[2].score - currentPlayer?.score 
    : 0
    ...
    <div className="bg-background text-on-background min-h-screen font-body-lg flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-gutter-mobile max-w-md mx-auto w-full py-6">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center font-display-md text-display-md shadow-sm mb-3 ${currentPlayer.color}`}>
          ...
          <div className="bg-primary-container/20 rounded-xl px-6 py-2">
            <span className="font-display-md text-display-md text-primary">{currentPlayer.score} pts</span>
          </div>
        </div>
          ...
          {playerRank > 3 && (
            <div className="bg-primary/10 border border-primary/30 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-label-lg text-label-lg ${currentPlayer.color}`}>
                  {currentPlayer.initial}
                </div>
                <span className="font-body-lg text-body-lg text-on-surface flex-1">
                  Toi
